...
//         if (device.name() == "BLE_Server") {  // Убедитесь, что имя совпадает с тем, что указано в сервере
//             qDebug() << "---Found BLE Server:" << device.name() << device.address().toString();
...
//         if (uuid == QBluetoothUuid("{00001234-0000-1000-8000-00805F9B34FB}")) {  // Используем тот же UUID, что и на сервере
...
//         if (targetServiceUuid.isNull()) {
//             qDebug() << "No suitable service found.";
//             return;
//         }

//         // Теперь подключаемся к службе
...
//         qDebug() << "Received confirmation from server:" << value;
//     }
